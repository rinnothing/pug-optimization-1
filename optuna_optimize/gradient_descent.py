...
def objective(trial):
    ...
    f_eps = trial.suggest_float("f_eps", 1e-9, 1e-5)
    c_1 = trial.suggest_float("wolfe_c1", 0.1, 0.6)
    c_2 = trial.suggest_float("wolfe_c2", 0.75, 1)
    max_count = trial.suggest_int("max_count", 1000, 10000)
    max_count_wolfe = trial.suggest_int("max_count_wolfe", 10, 40)

    lim = test_func.lim
    x = ((lim[1] - lim[0]) * 3) / 4  + lim[0]
    y = ((lim[1] - lim[0]) * 3) / 4  + lim[0]
    result = gr.gradient_descent(test_func.function, test_func.gradient,
